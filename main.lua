...
function addEmitter()
    ...
    if #emitters > 0 then
        ...
        emitter.object:setPosition((love.graphics.getWidth() + panelWidth)/2, love.graphics.getHeight()/2)
    end
...
function love.mousereleased(x, y, button)
    if button == 1 then guiElements.dragged = nil end
end
...
function love.wheelmoved(dx, dy)
    ...
    if hovered and hovered.value then
        local delta = love.keyboard.isDown("lshift") and hovered.modDelta or hovered.delta
        hovered.value = hovered.value + delta * dy
        updateValue(hovered)
    end
end
